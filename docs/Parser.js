...
const LoadValue = Parser.noop()
    .or(() => Parser.pure(LOAD_VAL).apl(r(/有數/)).ap(quoted.map(Number)).apl(period))
    .or(() => Parser.pure(LOAD_VAL)
    .apl(r(/有言曰/))
    .ap(quoted)
    .apl(period));
...
const SetMember = Parser.noop()
    ...
    .apl(r(/也/))
    .apl(period))
    .or(() => Parser.pure(SET_MEM_VAL).apl(r(/其/)).ap(quoted).ap(quoted).apl(period));
const MessageDefinition = Parser.noop().or(() => Parser.pure(MSG_DEF).apl(r(/聞/)).ap(quoted).apl(r(/而/)).ap(Block));
const MessageSend = Parser.noop()
    ...
    .apl(r(/之/))
    .apl(period));
const FunctionalCall = Parser.noop().or(() => Parser.pure(MSG_SEND(["window"]))
    .apl(r(/請/))
    .apl(ws)
    .ap(quoted)
    .apl(r(/之/))
